...
case class Matrix[T](vecStatus: Vector[Vector[Any]], vecRow: Vector[Vector[Any]], vecCol: Vector[Vector[Any]], list: Vector[Player], currentPlayer: Player):
  ...
  def colSize(vecIndex: Int, row: Int): Int = vector(vecIndex)(row).size
  def cell(vecIndex: Int, row: Int, col: Int): Any = vector(vecIndex)(row)(col)
  def row(vecIndex: Int, row: Int): Vector[Any] = vector(vecIndex)(row)
  ...
  def vector(vecIndex: Int): Vector[Vector[Any]] = this.productElement(vecIndex).asInstanceOf[Vector[Vector[Any]]]
  ...
  def isEdge(move: Move): Boolean = move.vec match
  ...
  def playerIndex: Int = list.indices.map(x => list(x).playerId).indexOf(currentPlayer.playerId)
