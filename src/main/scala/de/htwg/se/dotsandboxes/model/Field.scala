...
case class Field(matrix: Matrix[Any]):
    ...
    def put(vecIndex: Int, x: Int, y: Int, status: Any) =
        copy(matrix.replaceCell(vecIndex, x, y, status))
    def check(vecIndex: Int, x: Int, y: Int): Field =
        copy(matrix.checkMove(vecIndex, x, y))
